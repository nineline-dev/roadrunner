...
var trackSiteAnalyticsEvent = (event) => {
  ...
  if (shouldSendToPosthog(event.name)) {
    posthog3.capture(event.name, properties);
  }
...
var identifySiteAnalyticsPerson = (distinctId, properties) => {
  posthog3.identify(distinctId, sanitizePosthogProperties(properties));
};
var registerSiteAnalyticsContext = (properties) => {
  posthog3.register(sanitizePosthogProperties(properties));
};
  ...
  const dispatch = async (event) => {
    ...
    if (vendorConsentGranted && destinations.includes("posthog")) {
      ...
      if (!usedDirectCapture) {
        ...
        posthog4.capture(posthogEventName, posthogProperties, posthogCaptureOptions);
      }
    }
...
var registerPosthogContext = (config, client = posthog6) => {
  const context = resolveRuntimeEntityContext(config);
...
var mountPosthog = (config) => {
  ...
  if (replayEnabled) posthog6.startSessionRecording();
};
var optOutPosthog = () => {
  try {
    posthog6.stopSessionRecording();
...
var mountSiteAnalytics = (config) => {
  ...
  const handle = {
    ...
    identify: (distinctId, properties = {}) => {
      const identityContext = resolveRuntimeEntityContext(runtimeConfig);
      posthog6.identify(distinctId, sanitizeIdentityProperties(runtimeConfig, properties));
      if (identityContext.global_id) {
        posthog6.group("global_entity", identityContext.global_id, buildPosthogGlobalEntityGroupContext(runtimeConfig));
      }
      if (identityContext.customer_account_id) {
        posthog6.group("customer_account", identityContext.customer_account_id, buildPosthogCustomerAccountGroupContext(runtimeConfig));
      }
    },
