...
__export(index_exports, {
  ...
  DEFAULT_STOREFRONT_ANALYTICS_MODES: () => DEFAULT_STOREFRONT_ANALYTICS_MODES,
  GA4Provider: () => GA4Provider,
  PostHogDashboard: () => PostHogDashboard,
  PostHogProvider: () => PostHogProvider,
  RudderStackProvider: () => RudderStackProvider,
  STOREFRONT_EVENT_DEFINITIONS: () => STOREFRONT_EVENT_DEFINITIONS,
  STOREFRONT_EVENT_SPEC_VERSION: () => STOREFRONT_EVENT_SPEC_VERSION,
  STOREFRONT_REQUIRED_BASE_KEYS: () => STOREFRONT_REQUIRED_BASE_KEYS,
  SupersetDashboard: () => SupersetDashboard,
  UnifiedAnalyticsProvider: () => UnifiedAnalyticsProvider,
  analytics: () => analytics,
  applyStorefrontEventSink: () => applyStorefrontEventSink,
  ...
  createStorefrontDispatcher: () => createStorefrontDispatcher,
  getEnabledPacksForProfile: () => getEnabledPacksForProfile,
  getStorefrontEventDefinition: () => getStorefrontEventDefinition,
  getStorefrontEventDestinations: () => getStorefrontEventDestinations,
  identifyStorefrontPerson: () => identifyStorefrontPerson,
  isEventEnabledForProfile: () => isEventEnabledForProfile,
  mountStorefrontAnalytics: () => mountStorefrontAnalytics,
  registerStorefrontContext: () => registerStorefrontContext,
  resolveStorefrontAnalyticsModes: () => resolveStorefrontAnalyticsModes,
  setStorefrontReplay: () => setStorefrontReplay,
  trackStorefrontEvent: () => trackStorefrontEvent,
  useAnalytics: () => useAnalytics,
  useGA4: () => useGA4,
  usePostHog: () => usePostHog,
  useRudderStack: () => useRudderStack,
  validateStorefrontEvent: () => validateStorefrontEvent
});
  ...
  const [client] = (0, import_react.useState)(() => {
    ...
    return {
      ...
      page: (name, properties) => {
        import_posthog_js.default.capture("$pageview", {
...
var analytics = {
  ...
  page: (name, properties) => {
    import_posthog_js2.default?.capture("$pageview", {
      $current_url: window.location.href,
      $page_name: name,
      ...properties
    });
...
var trackStorefrontEvent = (event) => {
  ...
  import_posthog_js3.default.capture(toPosthogEventName(event.name), properties);
...
var identifyStorefrontPerson = (distinctId, properties) => {
  import_posthog_js3.default.identify(distinctId, properties);
};
var registerStorefrontContext = (properties) => {
  import_posthog_js3.default.register(properties);
};
...
var createStorefrontDispatcher = (config) => {
  ...
  const dispatch = async (event) => {
    ...
    if (destinations.includes("posthog")) {
      import_posthog_js4.default.capture(toPosthogEventName2(event.name), sanitizePosthogProperties2({
...
var mountPosthog = (config) => {
  ...
  import_posthog_js6.default.opt_in_capturing();
  import_posthog_js6.default.register({
    site_id: config.profile.site_id,
    customer_account_id: config.profile.customer_account_id,
    global_id: config.globalId ?? void 0,
    site_capabilities: config.profile.capabilities
  });
  import_posthog_js6.default.group("customer_account", config.profile.customer_account_id, {
    site_id: config.profile.site_id,
    global_id: config.globalId ?? void 0
  });
  if (config.profile.replay_enabled) import_posthog_js6.default.startSessionRecording();
};
var mountStorefrontAnalytics = (config) => {
  ...
  const handle = {
    ...
    identify: (distinctId, properties = {}) => import_posthog_js6.default.identify(distinctId, properties),
